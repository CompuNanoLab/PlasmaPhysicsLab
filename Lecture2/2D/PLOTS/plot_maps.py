...
for n, ts in enumerate(steps):
    ...
    im = ax[0].imshow(np.flip(rhoele, axis=1).T, cmap='Greys', norm=LogNorm(vmin=1e-3, vmax=rhoele.max()),
    ...
    # Plot ion density
    im = ax[1].imshow(np.flip(rhoion, axis=1).T, cmap='Blues', norm=LogNorm(vmin=1e-3, vmax=rhoion.max()),
                      extent=[x.min(), x.max(), y.min(), y.max()], interpolation='bicubic')
